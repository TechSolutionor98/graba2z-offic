...
const Checkout = () => {
  ...
  const processCardPayment = async (orderData) => {
    try {
      ...
      // Then initiate N-Genius payment - FIXED ENDPOINT
      const ngeniusResponse = await axios.post(
        `${config.API_URL}/api/payment/ngenius`,
        {
      ...
      // Store N-Genius reference in the order
      await axios.put(
        `${config.API_URL}/api/orders/${createdOrder._id}`,
        {
          paymentResult: {
            ngenius_order_ref: ngeniusResponse.data.orderReference,
            merchantOrderReference: ngeniusResponse.data.orderReference,
            status: "PENDING",
          },
        },
        {
      ...
      if (ngeniusResponse.data.paymentUrl) {
        window.location.href = ngeniusResponse.data.paymentUrl
      } else {
        throw new Error("Payment URL not received from N-Genius")
      }
    } catch (error) {
